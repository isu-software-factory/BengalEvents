...
RSpec.feature "EventDetails", type: :feature do
  context "create a new event detail" do
    before do
      ...
      @time1 = Time.now
      @time2 = Time.now
      @time_slot = @location.time_slots.build(start_time: @time1, end_time: @time2, interval: 60)
      @time_slot.save
    ...
    it "should successfully create an event detail" do
      ...
      visit new_occasion_event_event_detail_path(occasion_id: @occasion.id, event_id: @event.id)
      within("form") do
